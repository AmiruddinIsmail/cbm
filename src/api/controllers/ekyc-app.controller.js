...
exports.merchantCallBackURL = async (req, res, next) => {
  ...
  try {
    ...
    if (getStatus.statusCode != "00" && getStatus.statusDescription) {
      data.cancelReason += `${cancelReasonDate} ${getStatus.statusDescription} (${params.fpx_sellerOrderNo})\n`;
    }

    if (!compareName && orderUpdateStatus) {
      data.order_status = ORDER_STATUS.CANCELLED;
      data.cancelReason += `${cancelReasonDate} Unmatched Bank Name. (${params.fpx_sellerOrderNo})\n`;
    }
    ...
    if (data.status == 1 && data.order_status != ORDER_STATUS.CANCELLED && data.order_status != ORDER_STATUS.FAILED_PAYMENT && orderUpdateStatus) {
      data.paymentCheck = 1;
...
exports.secondAttemptView = async (req, res, next) => {
  const data = {
    ...
    url: `${process.env.renew_plus_url}/customer/create/auth/`,
    image: configVariable.failed,
    footer: `Thank you. For any enquiries, kindly contact the ReNew+ team at support@compasia.com`,
    order_status: 9,
    status_respond: 0,
    button: true,
    secondAttemptUrl: "",
