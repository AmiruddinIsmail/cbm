...
exports.merchantCallBackURL = async (req, res, next) => {
  ...
  try {
    ...
    if (params.curlec_method == "00") {
      ...
      data.order_status = data.status == 1 ? 8 : 3;
      orderUpdateStatus =
    ...
    //Instant Pay
      ...
      data.order_status = data.status == 1 ? 1 : 3;
    ...
    if (getStatus.statusCode != "00") {
      data.cancelReason = `${getStatus.statusDescription}`;
    }

    if (!compareName && orderUpdateStatus) {
      data.order_status = 3;
      data.cancelReason += `Unmatched Bank Name.`;
    }
    ...
    if (data.status == 1 && data.order_status != 3 && orderUpdateStatus) {
      data.paymentCheck = 1;
...
exports.generatePaymentLink = async (req, res, next) => {
  try {
    ...
    if (getRequestMandate) {
      if (getRequestMandate.attempt >= 5) {
...
exports.generateMandateLink = async (req, res, next) => {
  try {
    ...
    if (getRequestMandate) {
      if (getRequestMandate.attempt >= 2) {
