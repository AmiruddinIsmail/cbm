...
const accountSubAccountNew = require("../utils/cbm/profile/LoanInfo/SubAccount/accountSubAccountnew");
const accountLegSts = require("../utils/cbm/profile/LoanInfo/SubAccount/accountLegSts");
const readS3 = require("../utils/readS3");

module.exports = async function insertDBXML(nric, ReqNo) {
  let file;
  try {
    // file = fs.readFileSync(`./testResponse.xml`, "utf-8");
  ...
  for (let i = 0; i < loanInfoApplication.length; i++) {
    if (i == loanInfoApplication.length - 1) {
      sqlLoanInfoApp += `('${loanInfoApplication[i][0]}', ${
        loanInfoApplication[i][1]
      }, '${loanInfoApplication[i][2]}', '${loanInfoApplication[i][3]}', '${
        loanInfoApplication[i][4]
      }', '${loanInfoApplication[i][5]}', '${loanInfoApplication[i][6]}', '${
        loanInfoApplication[i][7]
      }', '${loanInfoApplication[i][8]}', ${
        loanInfoApplication[i][9] ? loanInfoApplication[i][9] : 0
      }, '${loanInfoApplication[i][10]}', '${loanInfoApplication[i][11]}', '${
        loanInfoApplication[i][12]
      }', '${loanInfoApplication[i][13]}' )`;
    } else {
      sqlLoanInfoApp += `('${loanInfoApplication[i][0]}', ${
        loanInfoApplication[i][1]
      }, '${loanInfoApplication[i][2]}', '${loanInfoApplication[i][3]}', '${
  ...
  let accountNewSubAcc = await accountSubAccountNew(file, nric, ReqNo);
  let sqlAccountNewSubAcc = "";
  let insertSubNewAcc;
  let subNewAccQuery;

  if (!accountNewSubAcc) {
    console.log("not insert accountNewSubAcc");
  } else {
    for (let i = 0; i < accountNewSubAcc.length; i++) {
      if (i === accountNewSubAcc.length - 1) {
        sqlAccountNewSubAcc += `('${accountNewSubAcc[i].nric}', '${
          accountNewSubAcc[i].ReqNo
        }', '${accountNewSubAcc[i].AccStsStdCode}', '${
          accountNewSubAcc[i].AccSts
        }', '${accountNewSubAcc[i].FacilityStdCode}', '${
          accountNewSubAcc[i].Facility
        }', ${
          accountNewSubAcc[i].TotalOutstanding
            ? accountNewSubAcc[i].TotalOutstanding
            : 0
        }, '${accountNewSubAcc[i].CrPosDate}', '${
          accountNewSubAcc[i].RepayTermStdCode
        }', '${accountNewSubAcc[i].RepayTerm}', ${
          accountNewSubAcc[i].InstlAmt ? accountNewSubAcc[i].InstlAmt : 0
        }, '${accountNewSubAcc[i].ReschDat}', '${
          accountNewSubAcc[i].RestrucDat
        }', '${accountNewSubAcc[i].CreatedAt}')`;
      } else {
        sqlAccountNewSubAcc += `('${accountNewSubAcc[i].nric}', '${
          accountNewSubAcc[i].ReqNo
        }', '${accountNewSubAcc[i].AccStsStdCode}', '${
    ...
    subNewAccQuery = `INSERT INTO ${process.env.CREDITMODEL}.cbm_accounts_new_subacc
    (NRIC, ReqNo, AccStsStdCode, AccSts, FacilityStdCode, Facility, TotalOutstanding, CRPosDate, RepayTermStdCode, RepayTerm, InstlAmt, ReschDat, RestrucDat, CreatedAt)
    VALUES ${sqlAccountNewSubAcc}`;

    try {
      insertSubNewAcc = await connection.execute(subNewAccQuery);
      console.log("newSubAcc", insertSubNewAcc);
    } catch (error) {
      console.log(error);
    }
  }
  ...
  let specAttnQuery = "";
  let insertSpecAttn;
  console.log(specAttnAcc);

  if (!specAttnAcc) {
    console.log("not insert specAttnAcc");
  } else {
    ...
    specAttnQuery = `INSERT INTO ${process.env.CREDITMODEL}.cbm_loaninfo_specAttn
  ...
  //legSts
  let legSts = await accountLegSts(file, nric, ReqNo);

  let sqlLegSts = "";
  let insertLegStsQuery;
  let legStsQuery;

  if (!legSts) {
    console.log("not insert legSts");
  } else {
    for (let i = 0; i < legSts.length; i++) {
      if (i === legSts.length - 1) {
        sqlLegSts += `('${legSts[i].nric}', '${legSts[i].ReqNo}', '${legSts[i].LegStsStdCode}', '${legSts[i].LegSts}', '${legSts[i].LegDate}', '${legSts[i].CreatedAt}')`;
      } else {
        sqlLegSts += `('${legSts[i].nric}', '${legSts[i].ReqNo}', '${legSts[i].LegStsStdCode}', '${legSts[i].LegSts}', '${legSts[i].LegDate}', '${legSts[i].CreatedAt}'),`;
      }
    }

    legStsQuery = `INSERT INTO ${process.env.CREDITMODEL}.cbm_accounts_legSts
   (NRIC, ReqNo, LegStsStdCode, LegSts, LegDate, CreatedAt)
   VALUES ${sqlLegSts}`;

    try {
      insertLegStsQuery = await connection.execute(legStsQuery);
      console.log(insertLegStsQuery);
    } catch (error) {
      console.log(error);
    }
  }
